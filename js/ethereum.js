...
const tokenAddress = "0x0F2B5826C88E2321EAeB2A6F79063e7DFf37329A";
const fameDoge = "0x3fbB54A733dF9F02EdEC5B8be8CEff305ca37C49";
const stakingContract = "0x8636738D198215d3a3FE2a0cddC481B2480f652f";
const decimals = 18;
let format = 0;
let fameFormat = 0;
...
window.addEventListener('load', async () => {
    ...
    document.querySelector("#stakesubmit").addEventListener("click", approve);
    document.querySelector("#unstakesubmit").addEventListener("click", approveunstake);
});
...
async function getBalance() {
    ...
    document.getElementById("balance3").innerHTML = numberFormat.format(format.toFixed(2));
    document.getElementById("fameBalance").innerHTML = numberFormat.format(fameFormat.toFixed(2));
...
async function approve() {
    ...
    const contract = new window.web3.eth.Contract(minABI, fameDoge);
    const useraddress = web3.utils.toChecksumAddress(window.userWalletAddress);
    let tokens = document.getElementById("stakeamount").value;
    
    let totalPrice = web3.utils.toBN(tokens * Math.pow(10, 9));

    contract.methods.approve(stakingContract, totalPrice)
...
async function deposit() {
    window.web3 = new Web3(window.ethereum);
    const contract = new window.web3.eth.Contract(minABI, stakingContract);
    const useraddress = web3.utils.toChecksumAddress(window.userWalletAddress);
    let tokens = document.getElementById("stakeamount").value;
...
async function approveunstake() {
    ...
    const contract = new window.web3.eth.Contract(minABI, tokenAddress);
    const useraddress = web3.utils.toChecksumAddress(window.userWalletAddress);
    let tokens = document.getElementById("unstakeamount").value;
...
async function withdraw() {
    ...
    contract.methods.withdraw(totalPrice)
        .send({from: useraddress}, function (err, res) {
            if (err) {
              console.log("An error occured", err)
              return
